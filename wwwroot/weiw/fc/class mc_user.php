<?php class mc_user
{
    public $uuid;              // 用户的唯一标识符
    public $name;              // 用户名
    public $money;             // 用户的金钱数额
    public $password;          // 用户的加密密码
    public $loginTime;         // 用户最近一次登录时间戳
    public $registerTime;      // 用户注册时间戳
    public $checkinTime;       // 用户最近一次签到时间戳
    public $isAllowedLogin;    // 用户是否被允许登录的标志
    public $CAPE;              // 用户的披风信息
    public $SKIN;              // 用户的皮肤信息
    public $loginToken;        // 用户的登录令牌
    public $accessToken;       // 用户的访问令牌
     ...
     */
    public function __construct($uuid, $name, $money)
    {
        $this->uuid = $uuid;
        $this->name = $name;
        $this->money = $money;
