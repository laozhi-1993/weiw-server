<?php class mc_auth
{
    ...
    public function __construct()
	{
		...
		if (isset($config['authServerUrl']) && $config['authServerUrl'])
		{
		...
		$this->capesDir              = dirname(dirname(__MKHDIR__)).'/usercapes/';
		$this->skinsDir              = dirname(dirname(__MKHDIR__)).'/userskins/';
		$this->implementationName    = 'weiw auth server';
		$this->implementationVersion = '1.1.3';
		...
		if (!is_dir($this->capesDir))
		{
			mkdir($this->capesDir, 0777, true);
		}
		
		if (!is_dir($this->skinsDir))
		{
			mkdir($this->skinsDir, 0777, true);
		}
    }
