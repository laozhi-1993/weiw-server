					...
					<tbody>
						...
						<tr foreach(var.fileserver_list.list,var.key,var.value)>
							...
							<td if(var.value.is_dir,==,true) class="file-actions">
								<svg if(var.fileserver_list.path,===,'') class="svg-icon pointer" onclick="window.location.href='console.php?name={echo:var.value.name}';window.parent.loading()"  viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg">
		...
		<script>
			...
			function uploadFile(file, path)
			{
				return new Promise((resolve, reject) => {
					...
					xhr.open("POST", `/weiw/index.php?mods=fileserver_upload&p=${encodeURIComponent('{echo:var.fileserver_list.path}/'+path)}`, true);
					xhr.send(formData);
				});
			}
			...
			function openDelete(name, path)
			{
				...
				document.querySelector('.btn-delete').onclick = () => {
					...
					fetch(`/weiw/index.php?mods=fileserver_delete&p=${path}&password=${password}`)
						.then(data => data.json())
						.then(data => {
							if (data.error) {
