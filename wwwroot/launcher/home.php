...
<html lang="zh-CN">
	...
	<body>
		...
		<section>
			<header class="header">
				<div class="user">
					<img src="/weiw/index_auth.php/avatar/{echo:var.mc_user.SKIN.hash}/48" alt="玩家头像" class="avatar" />
					<div>
						<h2>{echo:var.mc_user.name}</h2>
						<div class="money">金币：<span>{echo:var.mc_user.money}</span></div>
					</div>
				</div>

				<div class="button-container">
					<button class="game-button check-in"><span>📅 每日签到</span><span>✅ 已签到</span></button>
					<button class="game-button start-game">启动游戏</button>
				</div>
			</header>
			...
			<main class="preview-container">
				<div class="header-container">
					...
					<div>
						...
						<button class="uninstall-button uninstall-skin" onclick="resettingSkin()">
							<span>卸载皮肤</span>
						</button>
					</div>
				</div>
		...
		<script>
			...
			function resettingSkin()
			{
				fetch('/weiw/index.php?mods=mc_skin_resetting').then(() => location.reload()).catch(error => showMessage(error));
			}
