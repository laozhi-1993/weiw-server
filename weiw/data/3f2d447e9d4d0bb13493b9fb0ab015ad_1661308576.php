<?php defined('__MKHDIR__') or die(http_response_code(404)) ?><?php isset($this->_Unified['mc_user']['password']) or $this->_Unified['mc_user']['password'] = false ?>
<!DOCTYPE html>
	...
	<head>
		...
		<script>
			...
			function Refresh()
			{
				...
				{
					zhuangtai = false;
					$.getJSON("/weiw/index.php?<?php echo token ?>",{"mods":"mc_refresh_password"},function(result){
	...
	<body>
		<div class="password">
			<div class="margin">
				...
				<div class="kaiguan">
					<span title="密码已显示" onclick="password(true )"><?php if(file_exists("icons/show.svg") && is_file("icons/show.svg")) include("icons/show.svg") ?></span>
					<span title="密码已隐藏" onclick="password(false)"><?php if(file_exists("icons/hide.svg") && is_file("icons/hide.svg")) include("icons/hide.svg") ?></span>
				</div>
				<div class="text">
					<span id="hide">********************************</span>
					<span id="show"><?php echo $this->_Unified['mc_user']['password'] ?></span>
				</div>
				<div class="anniu">
