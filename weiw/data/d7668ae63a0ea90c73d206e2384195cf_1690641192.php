<?php defined('__MKHDIR__') or die(http_response_code(404)) ?><?php isset($this->_Unified['mc_user']['admin']) or $this->_Unified['mc_user']['admin'] = false ?><?php isset($this->_Unified['mc_user']['SKIN']['hash']) or $this->_Unified['mc_user']['SKIN']['hash'] = false ?><?php isset($this->_Unified['mc_user']['name']) or $this->_Unified['mc_user']['name'] = false ?><?php isset($this->_Unified['mc_user']['bi']) or $this->_Unified['mc_user']['bi'] = false ?><?php isset($this->_Unified['mc_user']['id']) or $this->_Unified['mc_user']['id'] = false ?><?php isset($this->_Unified['mc_user']['token']) or $this->_Unified['mc_user']['token'] = false ?>
<!DOCTYPE html>
<html lang="zh-cmn-Hans">
	<head>
		...
		<script>
			window.addEventListener('DOMContentLoaded', function() {
				$("#tab").on('load', () => {
					...
					scrollbarStyle += '::-webkit-scrollbar-track {border-radius:3px}';
					scrollbarStyle += '::-webkit-scrollbar-track {background-color:#fff}';
					scrollbarStyle += '::-webkit-scrollbar-thumb {border-radius:3px}';
					scrollbarStyle += '::-webkit-scrollbar-thumb {background-color:#999}';
					scrollbarStyle += 'body {overflow-y: scroll}';
					scrollbarStyle += 'body {padding-right: 5px}';
					scrollbarStyle += 'body {margin: 0}';
					scrollbarStyle += '</style>';
					
					
					$('#tab').contents().find("a:not([href^='javascript:'])").click(function() {
						$("#tab").hide();
						$("#tab-loading").show();
					});
					$('#tab').contents().find("head").prepend(scrollbarStyle);
					$("#tab").show();
					$("#tab-loading").hide();
				});
				
				
				$("main .menu ul li[data-url]").click(function() {
					$("main .menu ul li").removeClass("select");
					$(this).addClass("select");
					$("#tab").attr("src",$(this).data('url'));
					$("#tab").hide();
					$("#tab-loading").show();
				});
				
				
				$("main .menu ul li:eq(0)").trigger('click');
			});
			
			
			window.addEventListener("mc_state",(e) => {
				if(e.detail === 0)
				{
					$(".start").html('启动游戏')
				}
		...
		<style>
			...
			header .operate .signIn svg {
				padding: 12.5px 13px;
				padding-right: 2px;
				width: 20px;
				height: 20px;
			}
			header .operate .signIn svg path {
				fill: #7B68EE;
			}
			header .operate .signIn .money {
				display: inline-block;
				vertical-align: top;
				text-align: left;
				font-size: 20px;
			...
			header .operate .signIn .button:active {
	...
	<body>
		...
		<header>
			<div class="operate">
				<div class="user">
					<span id="avatar"><img src="/weiw/index_auth.php/avatar?size=48&hash=<?php echo $this->_Unified['mc_user']['SKIN']['hash'] ?>" /></span>
					<span id="name"><?php echo $this->_Unified['mc_user']['name'] ?></span>
				</div>
				<div class="signIn">
					...
					<span class="money"><?php echo $this->_Unified['mc_user']['bi'] ?></span>
					<span class="button">签到</span>
				</div>
				<div class="start" onclick="window.start({'username':'<?php echo $this->_Unified['mc_user']['name'] ?>','uuid':'<?php echo $this->_Unified['mc_user']['id'] ?>','token':'<?php echo $this->_Unified['mc_user']['token'] ?>'})">
					<span>启动游戏</span>
				</div>
			</div>
		</header>
		<main>
			<div class="menu">
				<ul>
					...
					<?php if($this->_Unified['mc_user']['admin']): ?><li data-url="users.php">
					...
					<?php if($this->_Unified['mc_user']['admin']): ?><li data-url="config.php">
					...
					<?php if($this->_Unified['mc_user']['admin']): ?><li data-url="console.php">
