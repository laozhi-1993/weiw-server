...
class Rcon {
	...
	{
		if(preg_match('!^Unknown command!',$list = $this->send('minecraft:list')))
		{
			$list = $this->send('list');
		}
		
		
		$list = preg_replace('!§[a-z0-9]{1}!','',$list);
		$list = preg_replace('!\x00!','',$list);
		...
		if(preg_match('!^[^0-9]*([0-9]+)[^0-9]*([0-9]+)!',$list,$number))
		{
			$data['current'] = $number[1];
			$data['maximum'] = $number[2];
		}
		
		
		if(preg_match('!(:|：)(.+)!',$list,$role))
		{
			$data['list'] = explode(',',$role[2]);
		}
